...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const proPlanBasePrice = 550;
  const orgPlanBasePrice = 3500;
  const proLicenseSeatThreshold = 50;
  const maxUsers = 2500;
  const maxUserPrice = 0.1;
  ...
  const apiSpecPricingTiers = [
    { apiCount: 125, price: 150 },
    { apiCount: 250, price: 125 },
    { apiCount: 1250, price: 105 },
    { apiCount: 2500, price: 20 },
    { apiCount: 3750, price: 5 },
    { apiCount: 5000, price: 3 },
    { apiCount: 6250, price: 1 },
  ];
  ...
  function getPricePerSpecCount(specCount) {
    var price = 0;
    var remainingSpecs = specCount;
    for (const tier of apiSpecPricingTiers) {
      if (remainingSpecs <= 0) break;
      price += (tier.price * Math.min(tier.apiCount, remainingSpecs)) / 1000;
      remainingSpecs = remainingSpecs - tier.apiCount;
    }
    return price;
  }

  function calculateUsageBill(
    apiSpecsSum,
    minLicenseCount,
    minAPICount,
    minBuildCount,
    ...
    const billableSpecCount = Math.max(apiCount, minAPICount);
    const billableBuildCount = Math.max(buildCount, minBuildCount);

    let apiSpecPrice = getPricePerSpecCount(billableSpecCount) * apiSpecsSum;

    let licensePrice = getPricePerLicense(billableLicenseCount);

    return licensePrice + apiSpecPrice + billableBuildCount;
  }
  ...
  selfPlan.addEventListener("click", function () {
    ...
    document.getElementById("selected-plan-type").value = "self";
  });

  proPlan.addEventListener("click", function () {
    toggleRecommendedPlan("team");
    document.getElementById("selected-plan-type").value = "team";
    sliders.forEach((slider) => setSlider(slider, "team"));
  });

  orgPlan.addEventListener("click", function () {
    toggleRecommendedPlan("organization");
    document.getElementById("selected-plan-type").value = "organization";
